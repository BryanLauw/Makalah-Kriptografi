...
static void analyse_update_cache( x264_t *h, x264_mb_analysis_t *a )
{
    ...
    static int global_bit_counter = 0; 
    static uint8_t embedded_map[8160] = {0}; // 120*68 = 8160
    
    if (h->fenc->i_frame == 0 && h->mb.i_mb_x == 0 && h->mb.i_mb_y == 0) {
        global_bit_counter = 0;
        memset(embedded_map, 0, sizeof(embedded_map));
    }

    // 2. Paksa P_SKIP menjadi P_L0 agar pesan tidak hilang
    if (h->mb.i_type == P_SKIP && stego_has_more_bits()) 
    {
        h->mb.i_type = P_L0;
        h->mb.i_partition = D_16x16;
    ...
    switch( h->mb.i_type )
    {
        ...
        case P_L0:
            switch( h->mb.i_partition )
            {
                case D_16x16:
                    {
                        // Memastikan mb yang diembed berbeda
                        int mb_index = h->mb.i_mb_y * h->mb.i_mb_stride + h->mb.i_mb_x;
                        
                        // Lewati injeksi jika sudah diembed
                        if (embedded_map[mb_index]) {
                            x264_macroblock_cache_ref( h, 0, 0, 4, 4, 0, a->l0.me16x16.i_ref );
                            x264_macroblock_cache_mv_ptr( h, 0, 0, 4, 4, 0, a->l0.me16x16.mv );
                            break;
                        }
                        ...
                        if (secret_bit != -1) 
                        {
                            ...
                            // File peta
                            FILE *f_map = fopen("stego_map.txt", "a");
                            if (f_map) {
                                fprintf(f_map, "%d %d %d\n", h->fenc->i_frame, 
